...
public class Customer extends User{
        ...
        public Customer(int ID,String username,String phoneNumber,String email,String password,String rank,double accuamount,int accucount) {
        	super(ID,username, "消费者", phoneNumber,email);
        ...
        // 交互界面
        public void showCustomerMenu(Manager manager1,Customer customer) {
	        ...
	        while (!exit) {
	            if (!loggedIn) {
	                ...
	                switch (choice) {
	                    case 1:
	                        register(manager1,customer);
	                        break;
	                    case 2:
	                        loggedInUser = login(manager1);
	                        loggedIn = true;
	                        break;
	                    case 3:
	                ...
	                System.out.println("5. 购票(若想知道想看的电影的所在的放映厅等场次信息，则先进行操作4)");
	                ...
	                switch (choice) {
	                    case 1:
	                        changePassword(manager1,loggedInUser);
	                        break;
	                    case 2:
	                        forgetPassword(manager1);
	                        break;
	                    case 3:
	                        viewMovieSchedule(manager1);
	                        break;
	                    case 4:
	                    	viewSelectedMovie(manager1);
	                        break;
	                    case 5:
	                        purchaseTickets(manager1,loggedInUser);
	                        break;
	                    case 6:
        ...
        // 注册
	    private static void register(Manager manager1,Customer customer) {
	        Scanner scanner = new Scanner(System.in);
	        ...
	        for(Customer user : manager1.customers) {
	        ...
	        customer = new Customer(ID,username,phoneNumber,email,password,"铜牌用户",0,0);
	        manager1.customers.add(customer);
	        System.out.println("成功注册!");
	        customer.login(manager1);
	    }
	    // 登录
	    private static Customer login(Manager manager1) {
	        Scanner scanner = new Scanner(System.in);
	        System.out.println("请输入用户名: ");
	        String username = scanner.nextLine();
	        Customer customer = findCustomer(manager1,username);
	        if (customer == null) {
	    ...
	    // 修改自身密码
	    private static void changePassword(Manager manager1,Customer customer) {
	        Scanner scanner = new Scanner(System.in);
	    ...
	    // 忘记密码
	    public void forgetPassword(Manager manager1) {
	        Scanner scanner = new Scanner(System.in);
	    ...
	    // 查看所有电影放映信息
	    private static void viewMovieSchedule(Manager manager1) {
	    ...
	    // 查看指定电影放映信息
	    private static void viewSelectedMovie(Manager manager1){
	    	Scanner scanner = new Scanner(System.in);
	        ...
	        for(Movie movie : manager1.movies) {
	        	if(movie.getTitle().equals(title)) {
	        		Set<Show>Shows = manager1.sessions.keySet();  // 根据输入的影片找到该影片放映的放映厅
	                for (Show show : Shows) {
	                    if ( movie.equals(manager1.sessions.get(show)) ) {
	    ...
	    // 购票
	    private void purchaseTickets(Manager manager1,Customer customer) {
	        Scanner scanner = new Scanner(System.in);
	        System.out.println("电影放映信息:");
	        System.out.println("------------------------------");
	        for (Movie movie : manager1.movies) {
	    ...
	    private static String selectSeats(Theater theater,int ticketCount) {
	        ...
	        for (int i = 0; i < theater.getSeats().length; i++) {
	            ...
	            for (int j = 0; j < theater.getSeats()[i].length; j++) {
	                System.out.print(theater.getSeats()[i][j] + "\t");
	            }
	            System.out.println();
	        }
	        System.out.println("请选择座位: " + ticketCount + " seats (e.g., 1,2 3,4 5,6): ");
	    ...
	    // 通过用户名查找用户
	    private static Customer findCustomer(Manager manager1,String username) {
