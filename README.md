...
4. 退出登录：
  ...
  【注】：由于放映厅和影片存在一对多的关系，即一个放映厅内一次只能放一部电影，而一部电影可以在多个放映厅进行放映，所以定义了一个哈希表sessions，其中Show类对象为键，Movie类对象为值。而放映厅和放映厅内的座位等信息是一一对应的关系，于是此处也用了一个哈希表theaters，其中Show类对象为键，Theater类对象为值。
   ...
   最后通过运行测试将一些错误输出或是错误输入后导致的错误修改了：
  1. 如运行视频中的输出空闲座位数>影院内的总座位数，最后我在Theater类中添加了计算空闲座位的计算方法。
  2. 如运行视频中向用户展示查看所有电影放映信息，每部电影被输出，会将其余所有已安排场次了的放映厅输出且无其余影片名称，通过查找发现相应的for循环中缺少了相应的if语句判断，最后解决
     后只会将每一个放映厅及其对应的影片输出一次。
